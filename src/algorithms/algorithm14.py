"""
=============================================================================
PROSPERITY 4 — ROUND 1 TRADER v14
=============================================================================
Products:
...
V14:
    Start from v11 and make one tiny execution change only.

    OSMIUM:
      - keep fair fixed at 10000
      - keep strong takes unchanged
      - keep passive quoting unchanged
      - only filter the weakest take trades:
          * buy at 9999 only if we are panic-short
          * sell at 10001 only if we are panic-long

    Rationale:
      The post-trade review showed v11 earns real money from strong takes and
      decent money from broad activity, but the very weakest fades are also
      where slippage and inventory churn can sneak in. This trims only the
      lowest-quality discretionary trades without changing the bot's overall
      behavior.
=============================================================================
"""
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            # =================================================================
                ...
                # Strong takes are unchanged.
                # Weakest fades at 9999 / 10001 are only allowed in panic unwind.
                # At panic: also trade at exactly 10000 to reduce position.

                if best_ask is not None:
                    for ask_price, ask_vol in bot_asks:
                        if ask_price < fair_int - 1 and take_buy_room > 0:
                            take_qty = min(-ask_vol, take_buy_room)
                            orders.append(Order(product, ask_price, take_qty))
                            take_buy_room -= take_qty
                            position += take_qty
                        elif ask_price == fair_int - 1 and take_buy_room > 0 and is_panic_short:
                            take_qty = min(-ask_vol, take_buy_room)
                            orders.append(Order(product, ask_price, take_qty))
                            take_buy_room -= take_qty
                            position += take_qty
                ...
                if best_bid is not None:
                    for bid_price, bid_vol in bot_bids:
                        if bid_price > fair_int + 1 and take_sell_room > 0:
                            take_qty = min(bid_vol, take_sell_room)
                            orders.append(Order(product, bid_price, -take_qty))
                            take_sell_room -= take_qty
                            position -= take_qty
                        elif bid_price == fair_int + 1 and take_sell_room > 0 and is_panic_long:
                            take_qty = min(bid_vol, take_sell_room)
                            orders.append(Order(product, bid_price, -take_qty))
                            take_sell_room -= take_qty
                            position -= take_qty
