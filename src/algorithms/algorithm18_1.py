"""
=============================================================================
PROSPERITY 4 — ROUND 1 TRADER v18.1
=============================================================================
ABLATION GOAL:
    Test only CHANGE 1 from algorithm18:
        OSMIUM one-sided book handling

WHAT IS ACTIVE:
    - Friend baseline Pepper logic
    - Friend baseline Osmium logic
    - NEW: if OSMIUM has only bids or only asks, still trade that visible side
      and still post the passive quote that is possible

WHAT IS NOT ACTIVE:
    - no spread-aware passive sizing
    - no weak-edge inventory filter

Why this ablation exists:
    algorithm18 improved overall, but it included 3 Osmium changes together.
    This file isolates whether "stop skipping one-sided books" is the piece
    that helped.
=============================================================================
"""
...
class Trader:
    ...
    def trade_osmium(self, order_depth: OrderDepth, position: int) -> list[Order]:
        ...
        # CHANGE 1 ONLY:
        # Friend baseline skipped when either side was missing.
        # This ablation only changes that behavior: skip only when BOTH sides
        # are missing.
        ...
        # Keep friend baseline quote pricing and full-size passive room.
