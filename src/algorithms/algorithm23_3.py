"""
=============================================================================
PROSPERITY 4 — ROUND 1 TRADER v23-3
=============================================================================
A/B TEST: Tighten PEPPER passive quote edge (MIN_EDGE 4 -> 3)

HYPOTHESIS:
    PEPPER passive quotes at fair +/- 4 may be too far from fair value
    to ever fill. Tightening to fair +/- 3 puts our quotes closer to
    the action, potentially catching more fills during the cycling slice
    (sell high, rebuy low around the core position).

    Since PEPPER is 70% of PnL and we're at 92% of theoretical, even
    a small improvement here (+50-100) helps toward the 10.8k target.

CHANGE FROM v23:
    - PEPPER_MIN_EDGE: 4 -> 3   (passive quotes 1 tick closer to fair)
    - Everything else identical to v23 (pressure overlay, OSMIUM, etc.)

EXPECTED IMPACT:
    +50 to +150 if tighter quotes catch more cycling fills
    -50 to -100 if tighter quotes get adversely filled
=============================================================================
"""
...
class Trader:
    ...
    # =========================================================================
    # PEPPER — only MIN_EDGE changed: 4 -> 3
    # =========================================================================
    PEPPER_LONG_TARGET = 80
    PEPPER_RES_K = 0.30
    PEPPER_MIN_EDGE = 3          # *** CHANGED: was 4 in v23 ***
    ...
    def trade_osmium(
        ...
        """Identical to v23 — no OSMIUM changes in this test."""
    ...
    def trade_pepper(self, order_depth: OrderDepth, position: int) -> list[Order]:
        """
        Identical to v23 PEPPER except MIN_EDGE = 3 (was 4).

        This means passive quotes are placed at:
            bid = min(best_bid + 1, floor(fair) - 3)   <-- was fair - 4
            ask = max(best_ask - 1, ceil(fair) + 3)     <-- was fair + 4

        One tick closer to fair on each side = more likely to fill.
        """
        bids, asks, best_bid, best_ask = self._book(order_depth)
        ...
        # *** THE ONLY LINE THAT MATTERS: MIN_EDGE = 3 instead of 4 ***
