"""
=============================================================================
PROSPERITY 4 — ROUND 2 TRADER 8
=============================================================================
Reset after the recent PEPPER variants became too eager to max out inventory
and then stop trading.

Main idea:
    - Keep the adaptive OSMIUM logic.
    - Simplify PEPPER into a lower-target, freer-recycling trader:
        * strong long bias remains
        * target is around 60 instead of 80
        * above target, the bot is much more willing to sell
        * below target, the bot reloads aggressively

This version intentionally gives up the idea that PEPPER should sit at +80
most of the day. The hypothesis is that persistent long bias plus active
recycling around a lower target can outperform "max long and freeze".
=============================================================================
"""
...
class Trader:
    ...
    PEPPER_RES_K = 0.16
    PEPPER_MIN_EDGE = 4
    PEPPER_BUY_BONUS = 2.0
    PEPPER_LIGHT_SELL_EDGE = 0.5
    PEPPER_HEAVY_SELL_EDGE = 1.5
    ...
    def trade_pepper(self, order_depth: OrderDepth, position: int, state: TradingState) -> list[Order]:
        ...
        if inventory < self.PEPPER_TARGET:
            buy_threshold += self.PEPPER_BUY_BONUS
        ...
        if inventory >= self.PEPPER_TARGET + 10:
            sell_threshold = fair_value + self.PEPPER_LIGHT_SELL_EDGE
        elif inventory >= self.PEPPER_TARGET:
            sell_threshold = fair_value + 1.0
        else:
            sell_threshold = fair_value + self.PEPPER_HEAVY_SELL_EDGE
        ...
        if sell_room > 0 and my_ask > 0 and inventory >= self.PEPPER_TARGET:
