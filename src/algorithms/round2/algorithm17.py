"""
=============================================================================
PROSPERITY 4 — ROUND 2 TRADER 17
=============================================================================
Narrow follow-up to algorithm14 for cross-dataset diagnosis.

Keep unchanged:
    - PEPPER logic from algorithm4 baseline
    - OSMIUM adaptive fair
    - Market Access Fee bid

Only intended change:
    OSMIUM becomes more selective in medium-spread regimes and is more willing
    to quote only the better side when inventory is already leaning.

Motivation:
    The two algorithm14 runs suggested that PEPPER is relatively stable across
    datasets, while OSMIUM edge quality swings more. This version tests whether
    weaker OSMIUM fills are coming from over-providing both sides in merely
    "okay" conditions.
=============================================================================
"""
...
class Trader:
    ...
    OSMIUM_WIDE_SPREAD = 19
    OSMIUM_MEDIUM_SPREAD = 13
    OSMIUM_TIGHT_SPREAD = 9
    OSMIUM_ONE_SIDE_MEDIUM_LEAN = 12
    ...
    def trade_osmium(self, order_depth: OrderDepth, position: int, trader_state: dict) -> tuple[list[Order], dict]:
        ...
        medium_spread = spread is not None and self.OSMIUM_MEDIUM_SPREAD <= spread < self.OSMIUM_WIDE_SPREAD
        quote_buy = buy_size > 0
        quote_sell = sell_size > 0

        # In medium spreads, stop auto-quoting both sides when inventory is
        # already leaning. Keep the side that helps inventory and suppress the
        # side that would deepen the lean.
        if medium_spread:
            if inventory >= self.OSMIUM_ONE_SIDE_MEDIUM_LEAN:
                quote_buy = False
            elif inventory <= -self.OSMIUM_ONE_SIDE_MEDIUM_LEAN:
                quote_sell = False

        if best_bid is not None:
            my_bid = min(best_bid + 1, math.floor(fair_value - inventory_shift) - 1)
            if quote_buy and my_bid > 0:
                orders.append(Order(OSMIUM, my_bid, buy_size))

        if best_ask is not None:
            my_ask = max(best_ask - 1, math.ceil(fair_value - inventory_shift) + 1)
            if quote_sell and my_ask > 0:
    ...
    def _scaled_passive_size(
        ...
        if spread is None:
            ...
            size = max(1, math.ceil(room * 0.42))
        elif spread >= self.OSMIUM_TIGHT_SPREAD:
            size = max(1, math.ceil(room * 0.12))
        else:
            size = max(1, math.ceil(room * 0.05))
