"""
=============================================================================
PROSPERITY 4 — ROUND 2 TRADER 12
=============================================================================
Follow-up to algorithm11.

Intent:
    Keep the same clean ablation structure:
      - PEPPER stays on the algorithm4 baseline
      - only OSMIUM sizing is changed

    Compared with algorithm11, this version is deliberately less conservative.
    The previous spread-regime experiment looked directionally valid but likely
    left too much good OSMIUM flow untaken.

OSMIUM regime idea here:
    - wide spread     -> near-full size
    - medium spread   -> more size than algorithm11
    - tight spread    -> still active, but small
=============================================================================
"""
...
class Trader:
    ...
    def _scaled_passive_size(
        ...
        if spread is None:
            ...
            size = max(1, math.ceil(room * 0.90))
        elif spread >= self.OSMIUM_MEDIUM_SPREAD:
            size = max(1, math.ceil(room * 0.62))
        elif spread >= self.OSMIUM_TIGHT_SPREAD:
            size = max(1, math.ceil(room * 0.28))
        else:
            size = max(1, math.ceil(room * 0.12))

        if side == "buy":
            if inventory > 40:
                size = max(1, math.ceil(size * 0.33))
            elif inventory < -40:
                size = max(1, math.ceil(size * 1.42))
        else:
            if inventory < -40:
                size = max(1, math.ceil(size * 0.22))
            elif inventory > 40:
                size = max(1, math.ceil(size * 1.30))
