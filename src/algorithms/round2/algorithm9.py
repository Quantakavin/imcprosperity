"""
=============================================================================
PROSPERITY 4 — ROUND 2 TRADER 9
=============================================================================
Focused follow-up to algorithm8.

Keep unchanged:
    - Market Access Fee bid
    - adaptive OSMIUM fair and spread-aware sizing

Main change:
    PEPPER should finally recycle more when inventory is already rich.

Hypothesis:
    The current PEPPER variants still drift back to +80 too easily because
    buying remains too permissive while selling at high inventory is still not
    aggressive enough. This version:

      - keeps the lower PEPPER target
      - reduces extra buy eagerness once already above target
      - lowers the sell thresholds meaningfully when inventory is high
      - makes passive PEPPER asks more available once comfortably long
=============================================================================
"""
...
class Trader:
    ...
    PEPPER_RES_K = 0.14
    PEPPER_MIN_EDGE = 4
    PEPPER_BUY_BONUS_BELOW_TARGET = 2.0
    PEPPER_SELL_EDGE_HIGH = 0.0
    PEPPER_SELL_EDGE_MID = 0.5
    PEPPER_SELL_EDGE_LOW = 1.5
    PEPPER_HIGH_INVENTORY = 68
    PEPPER_PASSIVE_SELL_FLOOR = 62
    ...
    def trade_pepper(self, order_depth: OrderDepth, position: int, state: TradingState) -> list[Order]:
        ...
        drift_anchor = 13_000.0 + state.timestamp * self.PEPPER_DRIFT_PER_TICK
        ...
        fair_value = 0.15 * drift_anchor + 0.85 * local_mid + imbalance_shift
        fair_value -= (position - self.PEPPER_TARGET) * self.PEPPER_RES_K
        ...
        if inventory < self.PEPPER_TARGET:
            buy_threshold += self.PEPPER_BUY_BONUS_BELOW_TARGET
        ...
        for bid_price, bid_volume in bids:
            ...
            if inventory >= self.PEPPER_HIGH_INVENTORY:
                sell_threshold = fair_value + self.PEPPER_SELL_EDGE_HIGH
            elif inventory >= self.PEPPER_TARGET:
                sell_threshold = fair_value + self.PEPPER_SELL_EDGE_MID
            else:
                sell_threshold = fair_value + self.PEPPER_SELL_EDGE_LOW
        ...
        if buy_room > 0 and my_bid > 0 and inventory < self.PEPPER_HIGH_INVENTORY:
            orders.append(Order(PEPPER, my_bid, buy_room))

        if sell_room > 0 and my_ask > 0 and inventory >= self.PEPPER_PASSIVE_SELL_FLOOR:
