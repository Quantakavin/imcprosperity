"""
=============================================================================
PROSPERITY 4 — ROUND 2 TRADER 5
=============================================================================
Main idea:
    - Keep the adaptive, inventory-aware OSMIUM logic from the stronger Round 2
      variants.
    - Upgrade PEPPER from a generic "core long" model to an explicit drift-line
      model: PEPPER appears to rise by roughly 1000 per day in the Round 2
      data, so trade deviations around that upward line rather than around only
      the local book mid.

Strategy split:
    - ASH_COATED_OSMIUM:
        spread capture around a slow adaptive fair
    - INTARIAN_PEPPER_ROOT:
        rising fair-value anchor + long core + residual trading around trend
=============================================================================
"""
...
class Trader:
    ...
    PEPPER_DAY_DRIFT = 1000.0
    PEPPER_TIME_DRIFT_PER_TICK = 0.01
    PEPPER_DAY_BASE_STEP = 1000.0
    PEPPER_MIN_EDGE = 4
    PEPPER_IMBALANCE_ALPHA = 0.8
    PEPPER_CORE_TARGET = 70
    PEPPER_MAX_TARGET = 80
    PEPPER_RES_K = 0.18
    PEPPER_RELOAD_BONUS = 2.0
    PEPPER_SELL_BUFFER = 3.0
    ...
    def trade_pepper(self, order_depth: OrderDepth, position: int, state: TradingState) -> list[Order]:
        bids, asks, best_bid, best_ask = self._book(order_depth)
        if best_bid is None or best_ask is None:
            return []

        drift_fair = self._pepper_drift_fair(state.timestamp)
        ...
        # Blend the structural drift-line with current book information, then
        # tilt back toward the desired long inventory.
        fair_value = 0.75 * drift_fair + 0.25 * local_mid + imbalance_shift
        fair_value -= (position - self.PEPPER_CORE_TARGET) * self.PEPPER_RES_K
        ...
        if inventory < self.PEPPER_CORE_TARGET:
            buy_threshold += self.PEPPER_RELOAD_BONUS

        for ask_price, ask_volume in asks:
            if buy_room <= 0 or ask_price > buy_threshold:
                break
        ...
        sell_threshold = fair_value + 1.0
        if inventory < self.PEPPER_CORE_TARGET:
            sell_threshold += self.PEPPER_SELL_BUFFER
        ...
        if buy_room > 0 and my_bid > 0 and inventory < self.PEPPER_MAX_TARGET:
            orders.append(Order(PEPPER, my_bid, min(buy_room, self.PEPPER_MAX_TARGET - inventory)))

        if sell_room > 0 and my_ask > 0 and inventory >= self.PEPPER_CORE_TARGET:
            orders.append(Order(PEPPER, my_ask, -sell_room))

        return orders

    def _pepper_drift_fair(self, timestamp: int) -> float:
        # Round 2 days are anchored roughly 1000 apart:
        #   day -1 -> around 11000
        #   day  0 -> around 12000
        #   day  1 -> around 13000
        #
        # In the simulator, each trading day uses timestamps 0..99900. The
        # current submission logs are for day 1, so a simple rising line from
        # 13000 by timestamp works as a usable structural fair estimate.
        return 13_000.0 + timestamp * self.PEPPER_TIME_DRIFT_PER_TICK
