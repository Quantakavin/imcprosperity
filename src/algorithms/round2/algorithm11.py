"""
=============================================================================
PROSPERITY 4 — ROUND 2 TRADER 11
=============================================================================
Clean ablation from algorithm4.

Only intended change:
    Stronger OSMIUM spread-regime sizing.

Why:
    OSMIUM has been consistently profitable, which suggests the broad market-
    making logic is sound. This version tests whether more explicit spread
    regimes improve results:

      - very wide spread -> quote larger
      - medium spread    -> quote moderate size
      - tight spread     -> quote small size only

PEPPER is kept exactly on the algorithm4 baseline.
=============================================================================
"""
...
class Trader:
    ...
    OSMIUM_WIDE_SPREAD = 18
    OSMIUM_MEDIUM_SPREAD = 12
    ...
    def _scaled_passive_size(
        ...
        if spread is None:
            ...
            size = max(1, math.ceil(room * 0.95))
        elif spread >= self.OSMIUM_MEDIUM_SPREAD:
            size = max(1, math.ceil(room * 0.50))
        elif spread >= self.OSMIUM_TIGHT_SPREAD:
            size = max(1, math.ceil(room * 0.18))
        else:
            size = max(1, math.ceil(room * 0.08))

        if side == "buy":
            if inventory > 40:
                size = max(1, math.ceil(size * 0.30))
            elif inventory < -40:
                size = max(1, math.ceil(size * 1.50))
        else:
            if inventory < -40:
                size = max(1, math.ceil(size * 0.20))
            elif inventory > 40:
                size = max(1, math.ceil(size * 1.40))
