"""
=============================================================================
PROSPERITY 4 — ROUND 2 TRADER 10
=============================================================================
Clean ablation from algorithm4.

Only intended change:
    PEPPER anti-rebuy at high inventory.

Why:
    The later PEPPER experiments suggest the repeated failure mode is not lack
    of long bias; it is that the bot keeps drifting back to +80 too easily.
    This version keeps algorithm4's overall shape, but suppresses PEPPER buys
    once inventory is already rich and makes selling slightly easier there.
=============================================================================
"""
...
class Trader:
    ...
    def trade_pepper(self, order_depth: OrderDepth, position: int) -> list[Order]:
        ...
        if inventory >= self.PEPPER_ANTI_REBUY_START:
            reload_threshold -= 2.0
        ...
        elif inventory >= self.PEPPER_ANTI_REBUY_START:
            sell_threshold -= 1.0
        ...
        if buy_room > 0 and my_bid > 0 and inventory < self.PEPPER_ANTI_REBUY_START:
