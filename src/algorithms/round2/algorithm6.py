"""
=============================================================================
PROSPERITY 4 — ROUND 2 TRADER 6
=============================================================================
Follow-up after algorithm5.

Main change:
    PEPPER is now managed as two conceptual layers:

    1. core long inventory:
       inventory we strongly prefer to keep

    2. trading sleeve:
       extra inventory above the core that we are willing to recycle when the
       market gets rich, then reload on dips

Rationale:
    The best runs so far suggest:
      - pure trend-following is too passive
      - pure local fair-value trading gives up the structural upward drift
      - the most likely sweet spot is a protected core plus active sleeve

OSMIUM:
    Keep the adaptive / inventory-aware market-making logic from the stronger
    recent versions.
=============================================================================
"""
...
class Trader:
    ...
    PEPPER_DRIFT_PER_TICK = 0.01
    PEPPER_IMBALANCE_ALPHA = 0.9
    PEPPER_MIN_EDGE = 4
    PEPPER_CORE_TARGET = 64
    PEPPER_SLEEVE_TARGET = 80
    PEPPER_CORE_RES_K = 0.14
    PEPPER_BUY_BONUS_BELOW_CORE = 2.0
    PEPPER_SELL_BUFFER_BELOW_CORE = 4.0
    PEPPER_SLEEVE_SELL_EDGE = 1.0
    PEPPER_FULL_SELL_EDGE = 5.0
    ...
    def trade_pepper(self, order_depth: OrderDepth, position: int, state: TradingState) -> list[Order]:
        ...
        drift_fair = 13_000.0 + state.timestamp * self.PEPPER_DRIFT_PER_TICK
        ...
        fair_value = 0.35 * drift_fair + 0.65 * local_mid + imbalance_shift
        fair_value -= (position - self.PEPPER_CORE_TARGET) * self.PEPPER_CORE_RES_K
        ...
        if inventory < self.PEPPER_CORE_TARGET:
            buy_threshold += self.PEPPER_BUY_BONUS_BELOW_CORE
        ...
        for bid_price, bid_volume in bids:
            ...
            if inventory > self.PEPPER_CORE_TARGET:
                sell_threshold = fair_value + self.PEPPER_SLEEVE_SELL_EDGE
            else:
                sell_threshold = fair_value + self.PEPPER_FULL_SELL_EDGE + self.PEPPER_SELL_BUFFER_BELOW_CORE
            ...
            if inventory <= self.PEPPER_CORE_TARGET:
                ...
                max_sell = min(max_sell, inventory - self.PEPPER_CORE_TARGET)

            if max_sell <= 0:
                break

            quantity = min(bid_volume, max_sell)
        ...
        if buy_room > 0 and my_bid > 0 and inventory < self.PEPPER_SLEEVE_TARGET:
            passive_buy = min(buy_room, self.PEPPER_SLEEVE_TARGET - inventory)
            if passive_buy > 0:
                orders.append(Order(PEPPER, my_bid, passive_buy))

        if sell_room > 0 and my_ask > 0 and inventory > self.PEPPER_CORE_TARGET:
            passive_sell = min(sell_room, inventory - self.PEPPER_CORE_TARGET)
            if passive_sell > 0:
