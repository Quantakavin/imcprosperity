"""
=============================================================================
PROSPERITY 4 — ROUND 2 TRADER 7
=============================================================================
Design goal:
    Keep the stronger adaptive OSMIUM logic, but simplify PEPPER after the
    recent over-constrained variants became "buy to 80 and stop trading".

PEPPER philosophy here:
    - strong long bias remains
    - drift is only a soft bias, not a hard fair anchor
    - inventory target is lower than max position, so the bot can still cycle
    - sells are allowed more readily when inventory is rich

This version is intentionally simpler than algorithm5/6 on the PEPPER side.
=============================================================================
"""
...
class Trader:
    ...
    PEPPER_RES_K = 0.24
    PEPPER_MIN_EDGE = 4
    PEPPER_BUY_BONUS = 1.5
    PEPPER_SELL_EXTRA_EDGE = 2.0
    PEPPER_PASSIVE_SELL_FLOOR = 58
    ...
    def trade_pepper(self, order_depth: OrderDepth, position: int, state: TradingState) -> list[Order]:
        ...
        fair_value = 0.2 * drift_anchor + 0.8 * local_mid + imbalance_shift
        ...
        if inventory < self.PEPPER_PASSIVE_SELL_FLOOR:
            sell_threshold += self.PEPPER_SELL_EXTRA_EDGE
