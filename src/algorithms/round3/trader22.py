"""
================================================================================
IMC PROSPERITY 4 — ROUND 3 — v22 (spot core + 5300/5400 spread as VFE bias)
================================================================================
...
THE STRATEGY HERE:
  ...
  4. Linked vouchers:
     ...
     trader20 -> sparse 5300/5400 spread: safer, still slightly negative
     trader21 -> slower asymmetric 5300/5400 spread: basically flat, but
                 still not a real alpha engine

     The new interpretation is:
       - the 5300/5400 spread may be more useful as INFORMATION than as a
         standalone trading object
       - extreme spread values looked related to subsequent VFE direction in
         the data
       - so this version stops direct 5300/5400 spread trading entirely and
         instead uses the spread to lightly skew VFE fair value

     Still left to try after this:
       - use the same spread to gate Hydrogel aggressiveness
       - try very small linked option market making around a local fair
       - revisit direct 5300/5400 trading only if the VFE-bias version is
         clearly additive first
  ...
  3. Vouchers:
     ...
     No direct 5300/5400 trading in this version; the "Link" clue is now
     expressed via a VFE fair-value skew instead.
...
class Trader:
    ...
    # Linked voucher information: only the core 5300/5400 relationship.
    ...
    LINK_WINDOW = 80
    LINK_MIN_SAMPLES = 35
    LINK_BIAS_SOFT_Z = 2.0
    LINK_BIAS_HARD_Z = 3.2
    LINK_BIAS_SOFT_TICKS = 1.0
    LINK_BIAS_HARD_TICKS = 2.0
    ...
    def run(self, state: TradingState):
        ...
        # ---- VFE: Kelp-style strategy (popular-mid as fair, lightly skewed
        #      by the linked 5300/5400 voucher spread) ----
        S = None
        if self.VFE in state.order_depths:
            ...
            if pop is not None:
                link_bias = self._linked_vfe_bias(state.order_depths, mem)
    ...
    # =========================================================================
    # LINKED VOUCHER SIGNAL -> VFE FAIR SKEW
    # =========================================================================
    def _linked_vfe_bias(
        self,
        order_depths: Dict[str, OrderDepth],
        mem: dict,
    ) -> float:
        """Use the 5300/5400 spread as a small VFE directional nudge.

        Rationale:
        - direct 5300/5400 spread trading was no longer blowing up, but it was
          still not adding enough positive PnL
        - the historical data suggested the spread carries some information
          about subsequent VFE direction
        - so in this version we consume the signal as information rather than
          as a standalone option position
        """
        mids = {}
        ...
        spread = mids["VEV_5300"] - mids["VEV_5400"]
        self._push_hist(mem["link_spread_5300_5400"], spread)
        if len(mem["link_spread_5300_5400"]) < self.LINK_MIN_SAMPLES:
            return 0.0

        stats = self._mean_std(mem["link_spread_5300_5400"])
        if not stats:
            return 0.0

        mean_spread, std_spread = stats
        z = (spread - mean_spread) / std_spread

        if z >= self.LINK_BIAS_HARD_Z:
            return -self.LINK_BIAS_HARD_TICKS
        if z >= self.LINK_BIAS_SOFT_Z:
            return -self.LINK_BIAS_SOFT_TICKS
        if z <= -self.LINK_BIAS_HARD_Z:
            return self.LINK_BIAS_HARD_TICKS
        if z <= -self.LINK_BIAS_SOFT_Z:
            return self.LINK_BIAS_SOFT_TICKS
