"""
================================================================================
IMC PROSPERITY 4 — ROUND 3 — v21 (spot core + slower asymmetric 5300/5400)
================================================================================
...
THE STRATEGY HERE:
  ...
  4. Linked vouchers:
     trader18 -> local smile residuals: too static, barely traded
     trader19 -> multi-spread / butterfly z-scoring: churned to death
     trader20 -> sparse 5300/5400 spread: much safer, but still mildly
                 negative because the spread edge appears slower than our
                 entry/exit horizon

     So this version keeps only one option idea alive, and slows it down:

       - trade just the 5300/5400 spread
       - enter only on tail dislocations relative to the spread's own recent
         history
       - prefer the historically stronger side of the spread
       - hold longer before exiting, because the data suggested reversion is
         real but slow
       - enforce a cooldown before re-entering
       - no 5500, no butterfly, no VFE hedge

     Still left to try after this:
       - use the 5300/5400 spread as a light VFE directional bias
       - try very small linked option market making around a local fair
       - revisit 5500 only if 5300/5400 becomes clearly positive first
...
class Trader:
    ...
    LINK_WINDOW = 80
    LINK_MIN_SAMPLES = 35
    LINK_SPREAD_EDGE_SHORT = 2.8   # preferred side: short 5300 / long 5400
    LINK_SPREAD_EDGE_LONG = 3.6    # opposite side only on much rarer extremes
    LINK_EXIT_BAND = 0.35
    LINK_COOLDOWN_TICKS = 4_000
    LINK_MIN_HOLD_TICKS = 8_000
    LINK_MAX_HOLD_TICKS = 25_000
    LINK_EXIT_STEP = 6
    ...
    def _ensure_link_memory(self, mem: dict) -> None:
        ...
        mem.setdefault("link_entry_ts", -10**9)
    ...
    def _linked_voucher_strategy(
        ...
        Important note from prior attempts:
        - trader19 overtraded this relation and paid spread repeatedly
        - trader20 was safer, but still a little too eager and symmetric
        This version is intentionally slower and asymmetric:
        - strongly prefer the historically cleaner "spread too high" side
        - hold longer before attempting to monetize reversion
        """
        mids = {}
        ...
        entry_ts = mem["link_entry_ts"]
        cooldown_done = (timestamp - last_exit) >= self.LINK_COOLDOWN_TICKS
        held_long_enough = (timestamp - entry_ts) >= self.LINK_MIN_HOLD_TICKS
        held_too_long = (timestamp - entry_ts) >= self.LINK_MAX_HOLD_TICKS

        # Exit existing spread only after a decent hold time. The historical
        # data suggests the relation mean-reverts, but more slowly than our
        # earlier option sleeves assumed.
        should_exit = False
        if state == -1 and held_long_enough and z_53_54 <= self.LINK_EXIT_BAND:
            should_exit = True
        elif state == 1 and held_long_enough and z_53_54 >= -self.LINK_EXIT_BAND:
            should_exit = True
        elif state != 0 and held_too_long:
            should_exit = True

        if should_exit:
            self._flatten_one_step("VEV_5300", order_depths, positions, results, self.LINK_EXIT_STEP)
            self._flatten_one_step("VEV_5400", order_depths, positions, results, self.LINK_EXIT_STEP)
            ...
            if net_5300 == 0 and net_5400 == 0:
                ...
                mem["link_entry_ts"] = -10**9
            return {k: v for k, v in results.items() if v}

        # Enter only when flat, outside cooldown, and on tail z-scores.
        # We strongly prefer the "spread too high" side because:
        # - it looked cleaner in the logs
        # - the data showed the opposite side was weaker / slower
        if state == 0 and cooldown_done:
            if z_53_54 >= self.LINK_SPREAD_EDGE_SHORT:
                self._sell_up_to("VEV_5300", order_depths, positions, results, self.LINK_VOUCHER_CAP, self.LINK_EXIT_STEP)
                self._buy_up_to("VEV_5400", order_depths, positions, results, self.LINK_VOUCHER_CAP, self.LINK_EXIT_STEP)
                if results["VEV_5300"] or results["VEV_5400"]:
                    mem["link_state"] = -1
                    mem["link_entry_ts"] = timestamp
            elif z_53_54 <= -self.LINK_SPREAD_EDGE_LONG:
                self._buy_up_to("VEV_5300", order_depths, positions, results, self.LINK_VOUCHER_CAP, self.LINK_EXIT_STEP)
                self._sell_up_to("VEV_5400", order_depths, positions, results, self.LINK_VOUCHER_CAP, self.LINK_EXIT_STEP)
                if results["VEV_5300"] or results["VEV_5400"]:
                    mem["link_state"] = 1
                    mem["link_entry_ts"] = timestamp
