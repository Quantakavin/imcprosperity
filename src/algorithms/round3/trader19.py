"""
================================================================================
IMC PROSPERITY 4 — ROUND 3 — v19 (spot core + rolling linked spreads)
================================================================================
...
THE STRATEGY HERE:
  ...
  4. Linked vouchers:
     trader18 proved that simply buying the "cheap-looking" 5400 and selling
     the "rich-looking" 5300 from a smile fit is not enough. The CSVs show
     that some of that shape is structural, not temporary mispricing.

     So this version trades the linked structure in a more local way:
       - track rolling spreads 5300-5400, 5400-5500, and the 5300/5400/5500
         butterfly
       - enter only when a spread is stretched relative to ITS OWN recent mean
       - exit as it reverts
       - keep sizes small and never hedge with VFE
...
class Trader:
    ...
    # Linked voucher sleeve: trade the local chain relationships explicitly.
    LINK_VOUCHERS = ["VEV_5300", "VEV_5400", "VEV_5500"]
    ...
    def _ensure_link_memory(self, mem: dict) -> None:
        ...
        mem.setdefault("link_spread_5400_5500", [])
        mem.setdefault("link_bfly_5300_5400_5500", [])
    ...
    def _linked_voucher_strategy(
        ...
        """Trade rolling mean reversion of linked voucher spreads.

        The clue suggests the important thing is the LINK between strikes, not
        a supposedly perfect absolute fair value for each option. So we track
        the chain's own relative-value quantities directly.
        """
        mids = {}
        ...
        s_54_55 = mids["VEV_5400"] - mids["VEV_5500"]
        butterfly = mids["VEV_5300"] - 2 * mids["VEV_5400"] + mids["VEV_5500"]

        self._push_hist(mem["link_spread_5300_5400"], s_53_54)
        self._push_hist(mem["link_spread_5400_5500"], s_54_55)
        self._push_hist(mem["link_bfly_5300_5400_5500"], butterfly)

        if len(mem["link_spread_5300_5400"]) < self.LINK_MIN_SAMPLES:
            return {}

        stats_53_54 = self._mean_std(mem["link_spread_5300_5400"])
        stats_54_55 = self._mean_std(mem["link_spread_5400_5500"])
        stats_bfly = self._mean_std(mem["link_bfly_5300_5400_5500"])
        if not stats_53_54 or not stats_54_55 or not stats_bfly:
            return {}

        mean_53_54, std_53_54 = stats_53_54
        mean_54_55, std_54_55 = stats_54_55
        mean_bfly, std_bfly = stats_bfly

        z_53_54 = (s_53_54 - mean_53_54) / std_53_54
        z_54_55 = (s_54_55 - mean_54_55) / std_54_55
        z_bfly = (butterfly - mean_bfly) / std_bfly

        results = {prod: [] for prod in self.LINK_VOUCHERS}

        # Pair 5300 vs 5400.
        if z_53_54 >= self.LINK_SPREAD_EDGE:
            # 5300 rich vs 5400: sell 5300, buy 5400
            self._sell_up_to("VEV_5300", order_depths, positions, results, self.LINK_VOUCHER_CAP, 6)
            self._buy_up_to("VEV_5400", order_depths, positions, results, self.LINK_VOUCHER_CAP, 6)
        elif z_53_54 <= -self.LINK_SPREAD_EDGE:
            self._buy_up_to("VEV_5300", order_depths, positions, results, self.LINK_VOUCHER_CAP, 6)
            self._sell_up_to("VEV_5400", order_depths, positions, results, self.LINK_VOUCHER_CAP, 6)
        elif abs(z_53_54) <= self.LINK_EXIT_BAND:
            # Spread normalized: trim back toward flat.
            self._flatten_one_step("VEV_5300", order_depths, positions, results, 4)
            self._flatten_one_step("VEV_5400", order_depths, positions, results, 4)

        # Pair 5400 vs 5500.
        if z_54_55 >= self.LINK_SPREAD_EDGE:
            # 5400 rich vs 5500
            self._sell_up_to("VEV_5400", order_depths, positions, results, self.LINK_VOUCHER_CAP, 6)
            self._buy_up_to("VEV_5500", order_depths, positions, results, self.LINK_VOUCHER_CAP, 6)
        elif z_54_55 <= -self.LINK_SPREAD_EDGE:
            self._buy_up_to("VEV_5400", order_depths, positions, results, self.LINK_VOUCHER_CAP, 6)
            self._sell_up_to("VEV_5500", order_depths, positions, results, self.LINK_VOUCHER_CAP, 6)
        elif abs(z_54_55) <= self.LINK_EXIT_BAND:
            self._flatten_one_step("VEV_5400", order_depths, positions, results, 4)
            self._flatten_one_step("VEV_5500", order_depths, positions, results, 4)

        # Butterfly only on stronger signal because it stacks the 5400 leg.
        if z_bfly >= self.LINK_BFLY_EDGE:
            # Wings rich, body cheap: short 5300/5500, long 5400
            self._sell_up_to("VEV_5300", order_depths, positions, results, self.LINK_VOUCHER_CAP, 4)
            self._buy_up_to("VEV_5400", order_depths, positions, results, self.LINK_VOUCHER_CAP, 8)
            self._sell_up_to("VEV_5500", order_depths, positions, results, self.LINK_VOUCHER_CAP, 4)
        elif z_bfly <= -self.LINK_BFLY_EDGE:
            self._buy_up_to("VEV_5300", order_depths, positions, results, self.LINK_VOUCHER_CAP, 4)
            self._sell_up_to("VEV_5400", order_depths, positions, results, self.LINK_VOUCHER_CAP, 8)
            self._buy_up_to("VEV_5500", order_depths, positions, results, self.LINK_VOUCHER_CAP, 4)
        elif abs(z_bfly) <= self.LINK_EXIT_BAND:
            self._flatten_one_step("VEV_5300", order_depths, positions, results, 2)
            self._flatten_one_step("VEV_5400", order_depths, positions, results, 4)
            self._flatten_one_step("VEV_5500", order_depths, positions, results, 2)
