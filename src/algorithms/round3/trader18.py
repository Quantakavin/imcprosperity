"""
================================================================================
IMC PROSPERITY 4 — ROUND 3 — v18 (spot core + linked voucher sleeve)
================================================================================
...
THE STRATEGY HERE:
  ...
  4. Linked vouchers:
     The image clue ("dangerous to go alone", "Link") and the data both point
     toward chain-aware pricing rather than isolated option punts. The Round 3
     CSVs show that VEV_5400 is persistently cheap relative to nearby strikes,
     while VEV_5300 / VEV_5500 are often rich relative to the chain.

     So we keep the spot core unchanged, then add a tiny take-only relative-
     value sleeve:
       - fit a local IV smile over 5200/5300/5400/5500
       - compare each live strike to the fitted linked fair value
       - trade only when the residual is clearly large
       - keep positions small, no passive quoting, no VFE delta hedge
...
class Trader:
    ...
    # Linked voucher sleeve: use the chain itself as the context instead of
    # trading single strikes alone.
    LINK_VOUCHERS = ["VEV_5300", "VEV_5400", "VEV_5500"]
    SMILE_FIT_VOUCHERS = ["VEV_5200", "VEV_5300", "VEV_5400", "VEV_5500"]
    ...
    def _linked_voucher_strategy(
        ...
        """Small take-only relative-value sleeve on the voucher chain.

        We do not try to market-make the options and we do not treat each
        strike in isolation. Instead:
          1. infer the current IV at 5200/5300/5400/5500 from their mids
          2. fit a local quadratic smile in moneyness
          3. compute residuals and linked fair prices for 5300/5400/5500
          4. buy cheap linked strikes / sell rich linked strikes in small size

        This is the first option logic we have that is explicitly built around
        the hidden "Link" clue and the residual pattern in the CSVs.
        """
        fit_points = []
        for prod in self.SMILE_FIT_VOUCHERS:
            ...
            z = implied_vol(m, S, K, T)
            if z is None:
                continue
            x = (K - S) / 100.0
            fit_points.append((prod, K, x, z))

        # Need enough points to define a local chain shape.
        if len(fit_points) < 4:
            return {}

        xs = [x for _, _, x, _ in fit_points]
        ys = [z for _, _, _, z in fit_points]
        a, b, c = self._fit_quadratic(xs, ys)
        ...
        for prod in self.LINK_VOUCHERS:
            ...
            if m is None:
                continue
            iv_now = implied_vol(m, S, K, T)
            if iv_now is None:
                continue

            x = (K - S) / 100.0
            fair_iv = a * x * x + b * x + c
            fair_price = bs_call(S, K, T, max(0.01, fair_iv))
            residual = iv_now - fair_iv
            pos = positions.get(prod, 0)
            cap = self.LINK_VOUCHER_CAP
            orders: List[Order] = []

            # Buy linked-cheap strikes, especially the historically cheap 5400.
            if residual <= -self.LINK_RESIDUAL_EDGE:
                for ask in sorted(od.sell_orders.keys()):
                    if ask >= fair_price - self.LINK_PRICE_EDGE:
                        break
                    if pos >= cap:
                        break
                    avail = -od.sell_orders[ask]
            ...
            # Sell linked-rich strikes in small size.
            if residual >= self.LINK_RESIDUAL_EDGE:
                for bid in sorted(od.buy_orders.keys(), reverse=True):
                    if bid <= fair_price + self.LINK_PRICE_EDGE:
                        break
                    if pos <= -cap:
                        break
                    avail = od.buy_orders[bid]
    ...
    def _fit_quadratic(self, xs: List[float], ys: List[float]):
        """Least-squares quadratic fit without extra dependencies."""
        n = len(xs)
        sx = sum(xs)
        ...
        # Solve the normal equations for ax^2 + bx + c.
        # [sx4 sx3 sx2][a]   [sx2y]
        # [sx3 sx2 sx ][b] = [sxy ]
        # [sx2 sx  n  ][c]   [sy  ]
        det = (
