"""
=============================================================================
PROSPERITY 4 — ROUND 1 TRADER v26.2
=============================================================================
Strategy fork:
    Conservative Pepper slice cycling expansion
...
WHY THIS VERSION EXISTS:
    ...
    The best run already makes some extra Pepper money by cycling a small slice
    around the +80 core long. The question is whether we can expand that
    slightly without giving up too much drift exposure.

    This fork is intentionally conservative:
      - OSMIUM stays exactly on the best algorithm18 logic
      - Pepper keeps the same fair model
      - we only give the slice a little more room

WHAT CHANGES FROM algorithm18:

    PEPPER:
      - hold floor lowered from 74 -> 72
      - active sells above the floor require bid >= fair + 3
      - active sells are capped to at most 6 units per tick
      - passive ask size is also capped to the cycle size

WHAT STAYS THE SAME:
    - OSMIUM unchanged
    - Pepper still reloads back toward +80
    - We never let the slice logic liquidate the core long
=============================================================================
"""
...
class Trader:
    # =========================================================================
    # OSMIUM — unchanged from algorithm18
    # =========================================================================
    ...
    # =========================================================================
    # PEPPER — conservative cycling expansion
    # =========================================================================
    ...
    # Slightly more room for the slice than algorithm18, but still conservative.
    PEPPER_HOLD_FLOOR = 72
    PEPPER_HOLD_EXTRA_EDGE = 2
    PEPPER_CYCLE_MAX = 6
    PEPPER_MIN_SELL_EDGE = 3
    ...
    def trade_pepper(self, order_depth: OrderDepth, position: int) -> list[Order]:
        ...
        # Buy side unchanged: still refill dips aggressively toward +80.
        for ask_price, ask_volume in asks:
        ...
        # Conservative active cycling on the sell side.
        cycle_sold = 0
        for bid_price, bid_volume in bids:
            if sell_room <= 0:
                break
            ...
            if inventory >= self.PEPPER_HOLD_FLOOR:
                # Above hold floor, only sell the slice at a clear premium.
                if bid_price <= fair_value + self.PEPPER_MIN_SELL_EDGE:
                    break
                max_cycle_remaining = self.PEPPER_CYCLE_MAX - cycle_sold
                max_from_floor = inventory - self.PEPPER_HOLD_FLOOR
                quantity = min(bid_volume, sell_room, max_cycle_remaining, max_from_floor)
                if quantity <= 0:
                    break
        ...
        # Passive ask is capped to the same cycle size so we do not dump too
        # much of the long core at once.
