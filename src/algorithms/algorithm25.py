"""
=============================================================================
PROSPERITY 4 — ROUND 1 TRADER v25
=============================================================================
Products:
    1. INTARIAN_PEPPER_ROOT  — core long (unchanged)
    2. ASH_COATED_OSMIUM     — v23 takes + refined flatten-at-fair

BASE: v24 (flatten-at-fair), fixing what went wrong.

WHAT CHANGED FROM v24:

    v24 used FLATTEN_THRESHOLD = 15. This was too aggressive:
    - v23's panic logic already sells at 10000 when position >= 30
    - Flattening at 16-29 was redundant and interfered with natural
      position management via profitable takes
    - Result: -19 vs v23 baseline

    v25 raises threshold to 40:
    - Only flatten when position is SERIOUSLY stuck (40+)
    - This targets the scenario the raw data showed: long consecutive
      runs of cheap asks that build position to 50-80 with no expensive
      bids to unwind naturally
    - Positions 30-39 are handled by existing panic logic
    - Positions 0-29 unwind naturally via profitable takes
    - Raw data simulation: threshold=40 scored 3,819 vs baseline 3,613
      (+206 theoretical uplift)

    Also removed FLATTEN_MAX_PER_TICK cap. When we're at +60 and see
    a bid at 10000, we should dump as much as possible to free room.
    The cap of 20 was overly conservative — if we're stuck at +60,
    selling 20 still leaves us at +40 which is still stuck.
...
class Trader:
    ...
    # Flatten-at-fair: only when seriously stuck.
    # 40 is above the panic threshold (30), so this only fires in the
    # range where panic alone isn't enough to free room fast enough.
    ...
    def trade_osmium(self, order_depth: OrderDepth, position: int) -> list[Order]:
        ...
        # =================================================================
        # Flatten at fair — BEFORE takes to free room
        #
        # Only fires when |position| > 40. At this level, we're seriously
        # stuck and need to shed inventory even at break-even to unlock
        # future profitable takes.
        #
        # No per-tick cap — dump as much as possible. If we're at +60
        # and there's a bid at 10000 for 30 units, sell all 20 excess
        # (down to threshold) to maximize freed room.
        # =================================================================
        ...
        # =================================================================
        # Profitable takes — identical to v23
        # =================================================================
