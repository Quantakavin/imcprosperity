"""
=============================================================================
PROSPERITY 4 — ROUND 1 TRADER v12
=============================================================================
Products:
    1. INTARIAN_PEPPER_ROOT  — linear drift +0.001/tick, buy+hold
    2. ASH_COATED_OSMIUM     — mean-reverts around 10,000 with strong imbalance

V12 CHANGES:
    PEPPER: unchanged from v11. Buy fast, hold forever.

    OSMIUM:
      1. Dynamic fair value = 10,000 + imbalance shift
         The Round 1 tape shows top-of-book imbalance predicts short-horizon
         direction much more strongly than static residual alone. We use the
         level-1 volume imbalance to shift fair by a few ticks instead of
         blindly fading every move back to exactly 10,000.

      2. Takes are filtered by dynamic fair, not fixed fair
         We still grab clear mispricings, but now we avoid fading a strongly
         one-sided book too early. In practice this means fewer dumb sells into
         rising pressure and fewer dumb buys into falling pressure.

      3. Passive size is clipped
         v11 could post the whole passive room at one price. That earns fills,
         but it also lets inventory balloon from speculative quotes. v12 only
         rests a modest clip per side and lets aggressive takes drive the larger
         inventory swings.

      4. Panic unwind is directional
         When inventory is stretched and the imbalance turns against us, we
         tighten the unwind side to 1 tick from fair and stop leaning into the
         bad side. If the book agrees with our inventory, we unwind less
         aggressively.
=============================================================================
"""
...
class Trader:
    ...
    # =========================================================================
    # OSMIUM — take + passive MM with imbalance-aware fair
    # =========================================================================
    OSMIUM_FAIR = 10000
    ...
    OSMIUM_QUOTE_OFFSET = 3   # base passive half-spread
    OSMIUM_SKEW_FACTOR = 4    # inventory skew

    # Shift fair by book imbalance.
    # Positive imbalance => more bid volume than ask volume => lean fair upward.
    OSMIUM_IMBALANCE_COEF = 4.0
    OSMIUM_IMBALANCE_SHIFT_CAP = 3

    # Only rest a moderate amount passively at one price.
    OSMIUM_PASSIVE_CLIP = 12

    # Strong directional unwind trigger.
    OSMIUM_PANIC_IMBALANCE = 0.25
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            # =================================================================
                ...
                # Book imbalance is the strongest extra signal in Round 1 Osmium.
                if best_bid is not None and best_ask is not None:
                ...
                imbalance_shift = round(self.OSMIUM_IMBALANCE_COEF * imbalance)
                imbalance_shift = max(-self.OSMIUM_IMBALANCE_SHIFT_CAP,
                                      min(self.OSMIUM_IMBALANCE_SHIFT_CAP, imbalance_shift))
                fair_int = self.OSMIUM_FAIR + imbalance_shift
                ...
                unwind_long_hard = is_panic_long and imbalance <= -self.OSMIUM_PANIC_IMBALANCE
                unwind_short_hard = is_panic_short and imbalance >= self.OSMIUM_PANIC_IMBALANCE

                # --- Layer 1: TAKE mispriced orders (full ±80 limit) ---
                # Buy any ask below dynamic fair. Sell any bid above dynamic fair.
                # At panic: also trade at exact fair when it reduces risk.
                ...
                # Add an imbalance lean on top of inventory skew.
                # Positive imbalance: quote a little higher. Negative: quote lower.
                bid_offset -= 0.75 * imbalance
                ask_offset -= 0.75 * imbalance
                ...
                if unwind_long_hard:
                    ask_offset = 1
                    bid_offset = max(bid_offset, self.OSMIUM_QUOTE_OFFSET + 1)
                elif unwind_short_hard:
                    bid_offset = 1
                    ask_offset = max(ask_offset, self.OSMIUM_QUOTE_OFFSET + 1)
                ...
                if passive_buy_room > 0:
                    quote_buy_qty = min(passive_buy_room, self.OSMIUM_PASSIVE_CLIP)
                    if not unwind_long_hard:
                        orders.append(Order(product, my_bid, quote_buy_qty))
                if passive_sell_room > 0:
                    quote_sell_qty = min(passive_sell_room, self.OSMIUM_PASSIVE_CLIP)
                    if not unwind_short_hard:
                        orders.append(Order(product, my_ask, -quote_sell_qty))
