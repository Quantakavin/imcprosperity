"""
=============================================================================
PROSPERITY 4 — ROUND 1 TRADER v13
=============================================================================
Products:
    1. INTARIAN_PEPPER_ROOT  — linear drift +0.001/tick, buy+hold
    2. ASH_COATED_OSMIUM     — mean-reverts around 10,000, take+passive MM

V13 GOAL:
    Keep the robust v11 structure, then add only one conservative idea from
    the v12 experiment: use imbalance to FILTER marginal Osmium trades, not to
    move fair value itself.

V13 CHANGES:
    1. PEPPER unchanged from v11.

    2. OSMIUM fair stays fixed at 10,000.
       The v12 dynamic-fair shift increased trading and worsened average prices.
       The fixed anchor is more robust.

    3. OSMIUM takes split into STRONG vs WEAK edges.
       - Strong edges are always taken:
           buy asks <= 9996, sell bids >= 10004
       - Weak edges are only taken if imbalance is not fighting us:
           buy 9997 only when the book is not strongly ask-heavy
           sell 10003 only when the book is not strongly bid-heavy
       This trims low-quality fades without touching the clearly profitable ones.

    4. Panic unwind is directional.
       At large inventory, unwind aggressively only when the imbalance turns
       against the position. Otherwise still reduce risk, but do not force the
       harshest flattening behavior.
=============================================================================
"""
...
class Trader:
    ...
    # V13: only use imbalance as a trade-quality filter.
    OSMIUM_WEAK_EDGE = 3
    OSMIUM_STRONG_EDGE = 4
    OSMIUM_IMBALANCE_BLOCK = 0.20
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            # =================================================================
                ...
                # Use top-of-book imbalance as a filter, not as a fair shift.
                if best_bid is not None and best_ask is not None:
                ...
                if best_ask is not None:
                    for ask_price, ask_vol in bot_asks:
                        edge = fair_int - ask_price
                        strong_buy = edge >= self.OSMIUM_STRONG_EDGE
                        weak_buy = edge >= self.OSMIUM_WEAK_EDGE and imbalance > -self.OSMIUM_IMBALANCE_BLOCK

                        if (strong_buy or weak_buy) and take_buy_room > 0:
                ...
                if best_bid is not None:
                    for bid_price, bid_vol in bot_bids:
                        edge = bid_price - fair_int
                        strong_sell = edge >= self.OSMIUM_STRONG_EDGE
                        weak_sell = edge >= self.OSMIUM_WEAK_EDGE and imbalance < self.OSMIUM_IMBALANCE_BLOCK

                        if (strong_sell or weak_sell) and take_sell_room > 0:
