# Ember Mushroom auction calculator
...
# - after auction, all bought inventory is sold at 20
# - total fee is 0.10 per unit traded (0.05 buy + 0.05 sell)
...
def find_clearing_price(all_bids, all_asks):
    ...
    for p in prices:
        ...
        if vol > best_volume or (vol == best_volume and p > best_price):
            best_volume = vol
            best_price = p
...
def my_fill_if_i_bid(my_price, my_qty, base_bids, base_asks):
    ...
    # Buyers strictly better than you get filled first
    better_bids = sum(q for p, q in all_bids.items() if p >
                      my_price and p >= clearing_price)

    # At your own price level, earlier orders get priority over you
